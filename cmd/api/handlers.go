...
func (app *Config) SearchInventory(w http.ResponseWriter, r *http.Request) {
	...
	go func(state_id, country_id, lga_id, text, limit, offset, category_id, subcategory_id, ulid, state_slug, country_slug, lga_slug, category_slug, subcategory_slug, user_id, product_purpose, user_slug, subdomain string) {
		// make the call via grpc
		...
		requestPayload.UserSlug,
		requestPayload.Subdomain,
	)
...
func (app *Config) DeleteSaveInventory(w http.ResponseWriter, r *http.Request) {
	...
	invServiceUrl := fmt.Sprintf("%s%s", os.Getenv("INVENTORY_SERVICE_URL"), "delete-saved-inventory")
...
func (app *Config) DeleteInventory(w http.ResponseWriter, r *http.Request) {
	...
	//extract the request body
	var requestPayload = DeleteInventoryPayload{
		InventoryId: id,
		UserId:      userId,
	}
...
func (app *Config) MyInventories(w http.ResponseWriter, r *http.Request) {
	...
	//extract the request body
	var requestPayload = MyInventoryPayload{
		UserId: userId,
		Page:   int32(page),
		Limit:  int32(limit),
	}
	...
	invServiceUrl := fmt.Sprintf("%s%s", os.Getenv("INVENTORY_SERVICE_URL"), "my-inventories")
